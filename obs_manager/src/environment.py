...
def get_environments():
    ...
    try:
        ...
        with open(env_path, 'r', encoding='utf-8') as f:
            ...
            basedir_str="".join(f.readlines(2)).replace('folder_snippet=','').rstrip()
            if len(vault_str) == 0 or len(basedir_str) == 0 or not os.path.isdir(vault_str) or not os.path.isdir(basedir_str):
